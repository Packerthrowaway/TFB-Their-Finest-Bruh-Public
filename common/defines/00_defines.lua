...
NDiplomacy = {
	...
	WARGOAL_WORLD_TENSION_REDUCTION = -0.25,			-- Reduction of pp cost for wargoal at 100% world tension, scales linearly
	...
	VOLUNTEERS_PER_COUNTRY_ARMY = 0.04,				-- Each army unit owned by the source country contributes this amount of volunteers to the limit.
	...
	MP_NUM_WINNERS_FOR_PEACE_PAUSE = 5,				-- Number of human players as winners needed for peace conference to pause and open for everyone in multiplayer, needs to fullfill MP_NUM_LOSERS_FOR_PEACE_PAUSE as well.
	MP_NUM_LOSERS_FOR_PEACE_PAUSE = 5,				-- Number of human players as losers needed for peace conference to pause and open for everyone in multiplayer, needs to fullfill MP_NUM_WINNERS_FOR_PEACE_PAUSE as well.
...
NCountry = {
	...
	MAJOR_MIN_FACTORIES = 300,						-- need at least these many factories to become a major
	...
	AIR_SUPPLY_CONVERSION_SCALE = 0.40,				-- Conversion scale for planes to air supply
...
NResistance = {
	...
	SUPPRESSION_NEEDED_BY_RESISTANCE_POINT = 0.50, -- Number of suppression point we need for each 1% of resistance
	...
	GARRISON_MANPOWER_LOST_BY_ATTACK = 0.02, 	-- Ratio of manpower lost by garrison at each attack on garrison (this number will be reduced by the hardness of garrison template)
...
NTechnology = {
	...
	BASE_YEAR_AHEAD_PENALTY_FACTOR = 6.0,		-- Base year ahead penalty
...
NMilitary = {
	...
	ARMY_LEADER_XP_GAIN_PER_UNIT_IN_COMBAT = 0.125, -- XP gain per unit in combat
	...
	PARACHUTE_FAILED_STR_DIV = 7.5,			   -- When the transport plane was shot down, we drop unit with almost NONE strenght
	PARACHUTE_DISRUPTED_EQUIPMENT_DIV = 1.3,	   -- When the transport plane was hit, we drop unit with reduced equipment. Penalty is higher as more hits was received (and AA guns was in the state).
	PARACHUTE_DISRUPTED_MANPOWER_DIV = 1.7,	       -- When the transport plane was hit, we drop unit with reduced manpower. Penalty is higher as more hits was received (and AA guns was in the state).
	PARACHUTE_DISRUPTED_STR_DIV = 2,			   -- When the transport plane was hit, we drop unit with reduced strength. Penalty is higher as more hits was received (and AA guns was in the state).
	PARACHUTE_PENALTY_RANDOMNESS = 0.05,			   -- Random factor for str,manpower,eq penalties.
	PARACHUTE_DISRUPTED_AA_PENALTY = 0.8,            -- How much the Air defence in the state (from AA buildings level * air_defence) is scaled to affect overall disruption (equipment,manpower,str).
	...
	EQUIPMENT_COMBAT_LOSS_FACTOR = 0.8,	 	       -- % of equipment lost to strength ratio in combat, so some % is returned if below 1
	...
	MIN_XP_RATE_TO_DECAY = 0.5,								-- minimum XP factor for dragged combats
	...
	ARMY_FUEL_COST_MULT = 0.75,						-- fuel cost multiplier for all army related stuff
	...
	OUT_OF_FUEL_SPEED_MULT = 0.1,					-- speed mult that armies get when out of fuel
...
NAir = {
	...
	COMBAT_MULTIPLANE_CAP = 2.5,						-- How many planes can shoot at each plane on other side ( if there are 100 planes we are atttacking COMBAT_MULTIPLANE_CAP * 100 of our planes can shoot )
	...
	NAVAL_STRIKE_DETECTION_BALANCE_FACTOR = 0.3,		-- Value used to scale the surface_visibility stats to balance the gameplay, so 100% detection chance still won't spam the strikes.
	...
	FUEL_COST_MULT = 0.7, -- fuel multiplier for all air missions
...
NNavy = {
	...
	CARRIER_STACK_PENALTY = 99,										-- The most efficient is 4 carriers in combat. 5+ brings the penalty to the amount of wings in battle.
	...
	ANTI_AIR_TARGETING = 0.5,                                       -- how good ships are at hitting aircraft
...
NAI = {
	...
	FUEL_RATIO_TO_EXIST_FUEL_SAVING_MODE = 0.10, 					-- countries will exit fuel saving mode if they have more fuel ratio than this
